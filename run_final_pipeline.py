...
def run_hybrid_system():
    ...
    for epoch in range(EPOCHS):
        ...
        # TQDM for Training
        train_bar = tqdm(train_loader, desc=f"Ep {epoch+1}/{EPOCHS} [Train]", leave=False)
        ...
        # TQDM for Validation
        val_bar = tqdm(val_loader, desc=f"Ep {epoch+1}/{EPOCHS} [Val]  ", leave=False)
        with torch.no_grad():
        ...
        # Print Stats (Train Loss and Val Loss)
        print(f"  Ep {epoch+1}: Train Loss {avg_t_loss:.4f} | Val Loss {avg_v_loss:.4f} | Val AUC {v_auc:.4f}")
