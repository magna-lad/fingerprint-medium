    ...
    # fingerprint image.
        ...
        for row in range(0, rows - self.block, self.block):
            for col in range(0, cols - self.block, self.block):
                ...
                if angle_block:
                    freq[row:row + self.block][:, col:col + self.block] = minutiaExtractor.frequest(image_block, angle_block, kernel_size,minWaveLength, maxWaveLength)
    ...
    def calculate_minutiaes(self, kernel_size=3):
        ...
        for i in range(1, x - kernel_size//2):
            for j in range(1, y - kernel_size//2):
                minutiae = minutiaExtractor.minutiae_at(biniry_image, j, i, kernel_size)
                if minutiae != "none":
